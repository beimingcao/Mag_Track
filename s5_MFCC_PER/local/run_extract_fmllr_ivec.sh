...
for x in train; do 
  dir=$data_fmllr/$x
  steps/nnet/make_fmllr_feats.sh --nj $train_nj --cmd "$train_cmd" --transform-dir $CV/exp/$mono_tri \
...
for x in test; do 
  dir=$data_fmllr/$x
  steps/nnet/make_fmllr_feats.sh --nj $decode_nj --cmd "$train_cmd" --transform-dir $CV/exp/$mono_tri/decode_test \
    $dir $CV/data/$x $gmmdir $dir/log $dir/data || exit 1 
