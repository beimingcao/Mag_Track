...
for x in train; do 
  dir=$data_fmllr/$x
  steps/nnet/make_basis_fmllr_feats.sh --nj $train_nj --cmd "$train_cmd" --transform-dir $CV/exp/$mono_tri \
    $dir $CV/data/$x $gmmdir $dir/log $dir/data || exit 1 
#  local/ivec/compute_vad_decision.sh --cmd "$train_cmd" --nj 8 $dir $CV/exp/make_vad $dir/data
done
for x in test; do 
  dir=$data_fmllr/$x
  steps/nnet/make_basis_fmllr_feats.sh --nj $decode_nj --cmd "$train_cmd" --transform-dir $CV/exp/$mono_tri/decode_test \
    $dir $CV/data/$x $gmmdir $dir/log $dir/data || exit 1 
#  local/ivec/compute_vad_decision.sh --cmd "$train_cmd" --nj 1 $dir $CV/exp/make_vad $dir/data
done
