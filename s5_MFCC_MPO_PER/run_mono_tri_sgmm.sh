...
timit=/home/sxh135130/work/silent
CV=$1$2
...
END
echo ============================================================================
echo "              tri3 : SAT Training & Decoding                 "
echo ============================================================================

# Align tri2 system with train data.
steps/align_si.sh --nj "$train_nj" --cmd "$train_cmd" \
 --use-graphs true $CV/data/train $CV/data/lang $CV/exp/tri1 $CV/exp/tri1_ali
...
for x in full diag; do
  steps/decode_fmllr.sh --nj "$decode_nj" --cmd "$decode_cmd" --fmllr_update_type $x \
...
steps/align_sgmm2.sh --nj "$train_nj" --cmd "$train_cmd" \
 --transform-dir $CV/exp/tri3_ali --use-graphs true --use-gselect true \
 $CV/data/train $CV/data/lang $CV/exp/sgmm2_4 $CV/exp/sgmm2_4_ali

steps/make_denlats_sgmm2.sh --nj "$train_nj" --sub-split "$train_nj" \
 --acwt 0.2 --lattice-beam 10.0 --beam 18.0 \
 --cmd "$decode_cmd" --transform-dir $CV/exp/tri3_ali \
 $CV/data/train $CV/data/lang $CV/exp/sgmm2_4_ali $CV/exp/sgmm2_4_denlats

steps/train_mmi_sgmm2.sh --acwt 0.2 --cmd "$decode_cmd" \
 --transform-dir $CV/exp/tri3_ali --boost 0.1 --drop-frames true \
 $CV/data/train $CV/data/lang $CV/exp/sgmm2_4_ali $CV/exp/sgmm2_4_denlats $CV/exp/sgmm2_4_mmi_b0.1
...
#   exp/sgmm2_4/decode_dev exp/sgmm2_4_mmi_b0.1/decode_dev_it$iter
  ...
  steps/decode_sgmm2_rescore.sh --cmd "$decode_cmd" --iter $iter \
   --transform-dir $CV/exp/tri3/decode_test $CV/data/lang_test_bg $CV/data/test \
   $CV/exp/sgmm2_4/decode_test $CV/exp/sgmm2_4_mmi_b0.1/decode_test_it$iter
done 
exit 0;
