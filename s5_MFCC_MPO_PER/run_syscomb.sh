...
timit=/home/sxh135130/work/MOCHA3_d10

: <<'END'
local/timit_data_prep_${CV}.sh $timit $CV || exit 1
...
for x in train test; do 
  steps/make_MotionVector.sh --cmd "$train_cmd" --nj $feats_nj $CV/data/$x $CV/exp/make_mfcc/$x $mfccdir
...
: <<'END'
steps/train_mono.sh  --nj "$train_nj" --cmd "$train_cmd" $CV/data/train $CV/data/lang $CV/exp/mono
...
steps/train_sat.sh --cmd "$train_cmd" \
 $numLeavesSAT $numGaussSAT $CV/data/train $CV/data/lang $CV/exp/tri2_ali $CV/exp/tri3
...
echo ============================================================================
echo "               System Combination (GMM-HMM + DBN HMM (Karel's recipe))    "
echo ============================================================================
...
#   exp/sgmm2_4_mmi_b0.1/decode_dev_it$iter exp/combine_2/decode_dev_it$iter
  ...
  local/score_combine.sh --cmd "$decode_cmd" \
   $CV/data/test $CV/data/lang_test_bg $CV/exp/mono/decode_test \
   $CV/exp/dnn4_pretrain-dbn-mono_dnn$iter/decode_test_dbn$iter $CV/exp/combine-mono-dbn$iter/decode_test
	echo mono-dbn$iter
  local/score_combine.sh --cmd "$decode_cmd" \
   $CV/data/test $CV/data/lang_test_bg $CV/exp/tri1/decode_test \
   $CV/exp/dnn4_pretrain-dbn-tri1_dnn$iter/decode_test_dbn$iter $CV/exp/combine-tri1-dbn$iter/decode_test
	echo tri1-dbn$iter
	: <<'END'
  local/score_combine.sh --cmd "$decode_cmd" \
   $CV/data/test $CV/data/lang_test_bg $CV/exp/mono/decode_test \
   $CV/exp/dnn4_pretrain-dbn-tri1_dnn$iter/decode_test_dbn$iter $CV/exp/combine-mono-tri1_dbn$iter/decode_test
	echo mono-tri-dbn$iter
  local/score_combine.sh --cmd "$decode_cmd" \
   $CV/data/test $CV/data/lang_test_bg $CV/exp/tri1/decode_test \
   $CV/exp/dnn4_pretrain-dbn-mono_dnn$iter/decode_test_dbn$iter $CV/exp/combine-tri1-mono_dbn$iter/decode_test
	echo tri-mono-dbn$iter 
END
done 
exit 0;
: <<'END'
