...
if [ $stage -le 0 ]; then
  ...
  # test
	...
	dir=$data_fmllr/test
  steps/nnet/make_delta_feats.sh --nj 1 --cmd "$train_cmd" \
  ...
  # Train the DNN optimizing per-frame cross-entropy.
    ...
    if [ $x == 1 ]; then
      ...
      ali=${srcdir}_ali
		...
		feature_transform=$CV/exp/dnn4_pretrain-dbn/final.feature_transform
		dbn=$CV/exp/dnn4_pretrain-dbn/${x}.dbn
...
if [ $stage -le -3 ]; then
  # Train the DNN optimizing per-frame cross-entropy.
	for x in `seq 1 6`; do
		dir=$CV/exp/dnn4_pretrain-dbn-${mono_tri}_dnn$x-drop
...
#      $data_fmllr/train $CV/data/lang $srcdir ${srcdir}_ali || exit 1;
      ali=${srcdir}_ali
		...
		# Train
		$cuda_cmd $dir/log/train_nnet.log \
			steps/nnet/train.sh --feature-transform $feature_transform --dbn $dbn --dropout-retention 0.5 --hid-layers 0 --learn-rate 0.008 \
			$data_fmllr/train_tr90 $data_fmllr/train_cv10 $CV/data/lang $ali $ali $dir || exit 1;
		# Decode (reuse HCLG graph)
		steps/nnet/decode.sh --nj 1 --cmd "$decode_cmd" --acwt 0.2 \
		  $gmmdir/graph $data_fmllr/test $dir/decode_test_dbn$x #|| exit 1;
...
if [ $stage -le 4 ]; then
  # Train the CNN+DNN optimizing per-frame cross-entropy.
	for x in 1 2; do		# `seq 1 6`
		dir=$CV/exp/dnn4_pretrain-dbn-${mono_tri}_cnn2_dnn$x
    ...
    if [ $x == 1 ]; then
      ...
      y=$((x-1))
      srcdir=$CV/exp/dnn4_pretrain-dbn-${mono_tri}_cnn2_dnn$y
      steps/nnet/align.sh --nj 1 --cmd "$train_cmd" \
      $data_fmllr/train $CV/data/lang $srcdir ${srcdir}_ali || exit 1;
      ali=${srcdir}_ali
		learn_rate=0.001
		...
		# Train
		$cuda_cmd $dir/log/train_nnet.log \
			steps/nnet/train.sh --feature-transform $feature_transform --prepend-cnn-type cnn1d --hid-layers $x --hid-dim 512 --splice 4 --learn-rate $learn_rate \
			$data_fmllr/train_tr90 $data_fmllr/train_cv10 $CV/data/lang $ali $ali $dir || exit 1;
		# Decode (reuse HCLG graph)
		steps/nnet/decode.sh --nj 1 --cmd "$decode_cmd" --acwt 0.2 \
		  $gmmdir/graph $data_fmllr/test $dir/decode_test_dbn$x #|| exit 1;
...
if [ $stage -le 4 ]; then
  ...
  steps/nnet/train_mpe.sh --cmd "$cuda_cmd" --num-iters 3 --acwt $acwt \
