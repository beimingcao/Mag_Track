...
for x in train; do 
  steps/make_MotionVector.sh --cmd "$train_cmd" --nj 1 $CV/data/$x $CV/exp/make_mv/$x $mvdir
  steps/compute_cmvn_stats.sh $CV/data/$x $CV/exp/make_mv/$x $mvdir
  steps/make_mfcc.sh --cmd "$train_cmd" --nj 1 $CV/data/$x $CV/exp/make_mfcc/$x $mfccdir
  steps/compute_cmvn_stats.sh $CV/data/$x $CV/exp/make_mfcc/$x $mfccdir
  steps/make_mfcc_MotionVector.sh --cmd "$train_cmd" --nj 1 $CV/data/$x $CV/exp/make_mfcc_mv/$x $mfccdir $mvdir $mfcc_mvdir
...
steps/train_sat.sh --cmd "$train_cmd" --train_tree false \
