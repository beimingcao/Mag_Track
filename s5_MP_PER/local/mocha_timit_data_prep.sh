...
[ -f $conf/mocha_spk.list ] || error_exit "$PROG: MOCHA Eval-set speaker list not found.";
...
if $uppercased; then
  tr '[:lower:]' '[:upper:]' < $conf/mocha_spk.list > $tmpdir/mocha_spk
else
  tr '[:upper:]' '[:lower:]' < $conf/mocha_spk.list > $tmpdir/mocha_spk
fi
