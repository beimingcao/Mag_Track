...
if [ $stage -le 2 ]; then
  # Train the DNN optimizing per-frame cross-entropy.
	for x in `seq 1 6`; do
		dir=$CV/exp/dnn4_nopretrain-dbn-${mono_tri}_dnn$x

    if [ $x == 1 ]; then
  		ali=${gmmdir}_ali
    else
      y=$((x-1))
      srcdir=$CV/exp/dnn4_nopretrain-dbn-${mono_tri}_dnn$y
      ...
      ali=${srcdir}_ali
		...
		$cuda_cmd $dir/log/train_nnet.log \
			steps/nnet/train.sh --hid-layers $x --hid-dim 512 --splice 4 --learn-rate 0.008 \
